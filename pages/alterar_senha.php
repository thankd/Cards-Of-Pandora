...
if(isset($_POST['cad_dados']) && $_POST['cad_dados'] == 'dados'){
	...
	$confirmar_nova_senha = $_POST['confirmar_senha'];
		...
		$senhaPesquisa = 0;
		$senhaPesquisaBanco = mysql_query("SELECT Senha FROM pessoa WHERE IdPessoa='$SecId'");
		$senhaPesquisa = mysql_fetch_row($senhaPesquisaBanco);
			
			if($nova_senha == $confirmar_nova_senha){
			if($senhaPesquisa[0]==md5($senha_atual)){
			$sql = mysql_query("UPDATE pessoa 
			SET Senha=md5('$nova_senha')
			WHERE IdPessoa='$SecId'");
				$result = "Senha Alterada com sucesso!!!";
			} else {
				$result = "A senha informada não é a senha atual!";	
			}
			} else { $result = "A Nova senha não foi confirmada corretamente!";	 }
...
<link href="Script/css/alt_senha.css" rel="stylesheet" type="text/css">
  ...
  <img src="../image/UserArea.png">    </div>
    ...
    <div class="login_color" id="bg_central">
        ...
        <p>&nbsp;</p>
          ...
          <br><label  for="senha_atual"> Senha Atual: </label>
          	  <input name="senha_atual" type="password" id="estiloC" size="21" class="txt bradius" required> <br>
          	  <label for="nova_senha"> Nova Senha: </label>
          	  <input name="nova_senha" type="password" maxlength="20" id="estiloC" size="21" class="txt bradius"  required> <br>
          	  <label for="confirmar_senha"> Confirmar Nova Senha: </label>
          	  <input name="confirmar_senha" type="password" maxlength="20" id="estiloC" size="21" class="txt bradius" required>   <br>
